...
class DateTimeUtility:
    ...
    @classmethod
    def convert_date_to_str(cls, date: datetime.datetime, fmt: str = "%Y%m%d%H%M%S"):
        """
        datetime.datetime 타입 날짜 정보를 문자열 형식으로 변환하여 반환합니다.
        :param date: 날짜 정보 datetime.datetime 인스턴스
        :param fmt: str: datetime 변환을 위한 format 정보
        :return:
        """
    ...
    @classmethod
    def convert_timedelta_to_numeric(cls, delta: datetime.timedelta, uom: str):
        """
        ...
        :return:
        """
        return delta / datetime.timedelta(**{cls.get_python_uom(uom): 1})
